    ...
    super::scoped::with_scoped_conn(&state, &app_handle, client_id.as_deref(), |conn| {
        ...
        let result: Result<InvoiceDetail> = (|| {
            conn.execute(
                ...
                params![id, payload.invoice_number, payload.invoice_type, payload.issue_date,
                    payload.due_date, payload.client_name, payload.client_email, payload.client_address,
                    payload.notes, subtotal_cents, tax_rate, tax_cents, total_cents, now],
            )?;
        ...
        let result: Result<InvoiceDetail> = (|| {
            ...
            if let Some(lines) = payload.lines {
                ...
                let (subtotal_cents, tax_cents, total_cents) =
                    compute_totals(&create_lines, tax_rate);
                conn.execute("UPDATE invoices SET tax_rate = ?1, subtotal_cents = ?2, tax_cents = ?3, total_cents = ?4, updated_at = ?5 WHERE id = ?6",
                    params![tax_rate, subtotal_cents, tax_cents, total_cents, now, id])?;
            }
