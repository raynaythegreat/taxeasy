...
pub fn get_cash_flow(
    ...
    crate::commands::scoped::with_scoped_conn(
        &state,
        Some(&app_handle),
        client_id.as_deref(),
        |conn| {
            ...
            let (ppe_start, ppe_end) =
                balance_range_by_role(conn, "equipment", &date_from, &date_to)?;
            ...
            let net_cash_from_investing: Decimal =
                investing_activities.iter().map(|l| l.amount).sum();
            ...
            let net_cash_from_financing: Decimal =
                financing_activities.iter().map(|l| l.amount).sum();
